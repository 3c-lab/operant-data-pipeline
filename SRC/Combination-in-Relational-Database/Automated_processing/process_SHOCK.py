...
df_existed = pd.read_csv('target_csv_for_existed_shock.csv')
...
def transform_shock(filepath):
    ...
    output_path = 'target_csv_for_shock_output/'
...
dfs = []
for folder in dbutils.fs.ls('target_csv_for_shock_input'):
        ...
        for f in dbutils.fs.ls(folder_path):
            ...
            if 'sa' in filepath:
                ...
                for ws in worksheets:
                    print(wb,ws)
                    dfs += [transform_old_shock(wb,ws)]
            else:
                if ('Backup' not in filepath) and ('SHOCK' in filepath):
                    print(filepath)
                    dfs += [transform_shock(filepath)]
