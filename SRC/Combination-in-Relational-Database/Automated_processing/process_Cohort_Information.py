...
MYPATH = 'target_csv_for_cocaine_cohort_information'
COCAINE_COHORT_FILES = [join(MYPATH, f) for f in listdir(MYPATH) if isfile(join(MYPATH, f))]
COCAINE_COHORT_FILES
...
# TODO: Need to update environment variables in (advanced settings => [create cluster] =>enter environment variables)
...
COCAINE_COHORT_DIR_FILEPATH = 'target_csv_for_cocaine_cohort_information'
OXY_COHORT_DIR_FILEPATH = 'target_csv_for_oxy_cohort_information'
...
SUBJECT_OUTPUT_FILEPATH = 'target_csv_for_cohort_subject.csv'
MEASUREMENT_OUTPUT_FILEPATH = 'target_csv_for_cohort_measurement.csv'
...
exit_tab_list = [
    'exit day', 'last good session', 'exit code', 'complete', 'tissue collected', 'exit notes', 'replaced by', 'Decision'
]
...
final_characteristics_list = [
    ...
    'age at surgery', 'date of death', 'days of experiment', 'exit day', 'last good session', 'exit code', 'complete', 'tissue collected', 'exit notes', 'replaced by', 'decision'
]
...
# for k, v in SUBJECT_INGEST_MAPPING.items():
#     print(k + ' --> ' + v)
...
RFID_OXY = rfid_df('target_csv_for_rfid_oxycodone.csv')
RFID_COC = rfid_df('target_csv_for_rfid_cocaine.csv')
