...
updates = pd.read_csv('target_csv_for_update_filelist_lga.csv')
UPDATE_LIST = list(updates['files'])
UPDATE_LIST

RFID_OXY = pd.read_csv('target_csv_for_rfid_oxycodone.csv', index_col=0)
RFID_COC = pd.read_csv('target_csv_for_rfid_cocaine.csv', index_col=0)
...
"""
for folder in dbutils.fs.ls('target_csv_for_lga_input'):
    if ('LGA_general_coc' in folder.path):
        folder_path = folder.path
        for f in tqdm(dbutils.fs.ls(folder_path)):
            filepath = "/" + f.path.replace(':','')
            if 'DISSECT' not in filepath and 'LGA04' in filepath:
                print(filepath)
                ...
                if 'M14' in df.Subject.tolist() or 'm14' in df.Subject.tolist():
                    print(filepath)
"""

"""
for folder in dbutils.fs.ls('target_csv_for_lga_input'):
    if ('LGA_general_oxy' in folder.path):
        folder_path = folder.path
        for f in tqdm(dbutils.fs.ls(folder_path)):
            filepath = "/" + f.path.replace(':','')
            if 'DISSECT' not in filepath and 'LGA12' in filepath:
                with open(filepath, "rb") as f:
                    file_content = f.read()
                # Load the binary data into a pandas DataFrame
                df_raw = pd.read_excel(io.BytesIO(file_content), engine='openpyxl').T
...
def transform_lga_sha(filepath):
    ...
    output_path = 'target_csv_for_lga_output/'
    ...
    if os.path.exists(os.path.join(output_path, fname + '.csv')) and f.name.split('/')[-1] not in UPDATE_LIST:
        ...
        return pd.read_csv(os.path.join(output_path, fname + '.csv'))
    print(filepath)
    # Load the binary data into a pandas DataFrame
    df_raw = pd.read_excel(io.BytesIO(file_content), engine='openpyxl').T
    df_raw.reset_index(inplace=True)
...
def transform_old_lga_sha(wb, ws):
    ...
    if os.path.exists(os.path.join(output_path, filename + '.csv')):
        print(wb+' : '+ws+' skipped')
        return pd.read_csv(os.path.join(output_path, filename + '.csv'))
    print(wb+' : '+ws)
        ...
        for f in dbutils.fs.ls(folder_path):
            ...
            if 'OLD_SA' in filepath:
                ...
                for ws in worksheets:
                    dfs += [transform_old_lga_sha(wb, ws)]
            else:
                if 'DISSECT' not in filepath:
                    dfs += [transform_lga_sha(filepath)]

final = pd.concat(dfs)
def pk_gen(row):
    return str(row['rfid'])+row['trial_id']+row['drug']
final['pk'] = final.apply(pk_gen, axis=1)
if len(final) != final['pk'].nunique() or final['rfid'].isna().sum()>0:
    wrongs = final['pk'].value_counts()[final['pk'].value_counts()>1]
    print("duplicates: ")
    for i in wrongs.index:
        print(final[final['pk']==i][['room','cohort','subject','trial_id','drug']])
    print("null rfid: ")
    print(final[final['rfid'].isna()][['room','cohort','subject','trial_id','drug']])
    if final['rfid'].isna().sum()==0:
        raise Exception("Duplicate Data")
    elif len(final) == final['pk'].nunique():
        raise Exception("Null Data")
    else:
        raise Exception("Dup data nad Null data")
