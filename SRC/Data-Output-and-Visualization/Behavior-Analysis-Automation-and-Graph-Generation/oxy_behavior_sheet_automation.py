...
def sort_columns_by_date(wb, source_sheet_name="Behavior Data", sorted_sheet_name="Sorted Data"):
    ...
    # Because columns 1..3 => Rat, RFID, Experiment Group, session columns start at 4
    columns_info = []
    ...
    # Sort ascending by the parsed date
    columns_info.sort(key=lambda x: x[1])

    # Copy columns A,B,C as is (Rat, RFID, Drug Group, Experiment Group)
    for r in range(1, max_row + 1):
        for c in [1, 2, 3, 4]:
            sorted_ws.cell(row=r, column=c, value=source_ws.cell(row=r, column=c).value)

    # Copy sorted columns to the new sheet, starting at column=4
...
def main():
    # -------------------------------------------------------------------------
    # 0) COHORT NUMBER: Set it here
    # -------------------------------------------------------------------------
    cohort_number = 26  # <--- ADJUST HERE
    cohort_str = f"C{cohort_number:02d}"  # e.g. "C21", "C07", etc.

    # Output file location:
    # "C:\Users\georg\George Lab Dropbox\George_Lab\Team GWAS\2. Oxycodone GWAS\Oxy Behavior Sheet Auto\Oxycodone_C25_behavior_sheet.xlsx"
    behavior_sheet_path = rf"C:\Users\georg\George Lab Dropbox\George_Lab\Team GWAS\2. Oxycodone GWAS\Oxy Behavior Sheet Auto\Oxycodone_{cohort_str}_behavior_sheet.xlsx"

    # -------------------------------------------------------------------------
    # 1) Cohort Info location
    #    e.g. "Oxycodone CXX Updated Cohort Information.xlsx"
    # -------------------------------------------------------------------------
    cohort_info_folder = r"C:\Users\georg\George Lab Dropbox\George_Lab\Team GWAS\2. Oxycodone GWAS\Oxy Cohort Information"
    cohort_file_name = f"Oxycodone {cohort_str} Updated Cohort Information.xlsx"
    cohort_file_path = os.path.join(cohort_info_folder, cohort_file_name)

    # 2) "Oxy Daily Issues" folder
    #    e.g. "Oxy_CXX_Issues.xlsx"
    daily_issues_folder = r"C:\Users\georg\George Lab Dropbox\George_Lab\Team GWAS\2. Oxycodone GWAS\Oxy Daily Issues"
    daily_issues_file_name = f"Oxy_{cohort_str}_Issues.xlsx"
    daily_issues_file_path = os.path.join(daily_issues_folder, daily_issues_file_name)

    # 3) "excel_output_files" folder
    #    "C:\Users\georg\George Lab Dropbox\George_Lab\Experiments\DataStream\DataSource\excel_output_files"
    #    inside that is "OXYCODONE" => subfolders LGA, SHA, PR
    oxy_folder_path = r"C:\Users\georg\George Lab Dropbox\George_Lab\Experiments\DataStream\DataSource\excel_output_files"
    excel_output_files = os.path.join(oxy_folder_path, "OXYCODONE")

    # -------------------------------------------------------------------------
    # 2) OPEN OR CREATE THE WORKBOOK; CLEAR SHEET IF EXISTS
    # -------------------------------------------------------------------------
    import openpyxl
    ...
    if os.path.exists(cohort_file_path):
        ...
        print(f"Warning: Oxy Cohort info file not found at: {cohort_file_path}")

    # -------------------------------------------------------------------------
    # Put "Rewards" in A5
    # -------------------------------------------------------------------------
    if not behavior_ws["A5"].value:
        behavior_ws["A5"] = "Rewards"

    # -------------------------------------------------------------------------
    # RAT, RFID, Drug Group, Experiment Group from "Timeline"
    # -------------------------------------------------------------------------
    if timeline_ws:
        if behavior_ws["A6"].value != "Rat":
            behavior_ws["A6"] = "Rat"
            behavior_ws["B6"] = "RFID"
            behavior_ws["C6"] = "Drug Group"
            behavior_ws["D6"] = "Experiment Group"

            # Find columns "RFID" and "Drug Group" and "Experiment Group"
            rfid_col_idx = None
            drug_group_col_idx    = None
            experiment_group_col_idx = None
            header_row = list(timeline_ws[1])
            for cidx, cell in enumerate(header_row, start=1):
                if cell.value == "RFID":
                    rfid_col_idx = cidx
                if cell.value == "Experiment Group":
                    experiment_group_col_idx = cidx
                if cell.value == "Drug Group": 
                    drug_group_col_idx = cidx

            row_offset = 1
            for row_data in timeline_ws.iter_rows(min_row=2):
                rat_val = row_data[0].value  # RAT is col=1 => index=0
                # RFID
                rfid_val = None
                ...
                # Experiment Group
                experiment_val = None
                if experiment_group_col_idx is not None and (experiment_group_col_idx - 1) < len(row_data):
                    experiment_val = row_data[experiment_group_col_idx - 1].value
                ...
                behavior_ws.cell(row=new_row, column=4, value=experiment_val)
                row_offset += 1
        else:
            print("Rat row already set. Make sure 'Experiment Group' column is also set if needed.")
    else:
        print("Warning: Timeline not loaded; cannot copy Rat/RFID/experimentGroup.")

    # Gather RAT/RFID/experiment from row=7 downward
    ...
    while True:
        ...
        if rat_val is None:
            break
        if rat_val not in ["Rat", "Active Lever Presses", "Inactive Lever Presses", "breakpoint"]:
            rfid_val = behavior_ws.cell(row=row_idx, column=2).value
            drug_val  = behavior_ws.cell(row=row_idx, column=3).value
            experiment_val = behavior_ws.cell(row=row_idx, column=4).value
            original_rat_rfid_dg.append((rat_val, rfid_val, drug_val, experiment_val))
        row_idx += 1

    # -------------------------------------------------------------------------
    # Insert Exit Day/Code/Notes in row=6 (after col=3)
    # -------------------------------------------------------------------------
    last_col_for_exit = 4
    ...
    behavior_ws.cell(row=6, column=last_col_for_exit + 4, value="Last Good Session")

    exit_day_col = last_col_for_exit + 1
    exit_code_col = last_col_for_exit + 2
    exit_notes_col = last_col_for_exit + 3
    last_good_col  = last_col_for_exit + 4

    # -------------------------------------------------------------------------
    # Build exit_data_map from 'Exit Tab'
    # -------------------------------------------------------------------------
    exit_data_map = {}
    if exit_tab_ws:
        exit_tab_headers = {}
        for cidx, cell in enumerate(exit_tab_ws[1], start=1):
            val = cell.value
            if val:
                exit_tab_headers[str(val).strip()] = cidx
        ...
        last_good_idx = exit_tab_headers.get("Last Good Session")

        if rat_col_idx_exit and exit_day_idx and exit_code_idx and exit_notes_idx and last_good_idx:
            for row_i in range(2, exit_tab_ws.max_row + 1):
                rat_val_exit = exit_tab_ws.cell(row=row_i, column=rat_col_idx_exit).value
                if rat_val_exit:
                    day_v = exit_tab_ws.cell(row=row_i, column=exit_day_idx).value
                    code_v = exit_tab_ws.cell(row=row_i, column=exit_code_idx).value
                    notes_v = exit_tab_ws.cell(row=row_i, column=exit_notes_idx).value
                    last_good_v = exit_tab_ws.cell(row=row_i, column=last_good_idx).value
                    exit_data_map[str(rat_val_exit).strip()] = (day_v, code_v, notes_v, last_good_v)
        else:
            print("Warning: Exit columns missing in exit tab.")
    else:
        print("Warning: Exit tab not loaded properly.")

    # -------------------------------------------------------------------------
    # SUBFOLDERS: LGA, SHA, PR (no SHOCK for Oxy)
    # Collect session names from files that start with MTF134 or BSB273,
    # contain the correct cohort code, and match "HSOXY(\w+)_output.xlsx"
    # -------------------------------------------------------------------------
    cohort_pattern = re.compile(rf"{cohort_str}")
    session_pattern = re.compile(r"HSOXY(\w+)_output\.xlsx$")
    subfolders = ["LGA", "SHA", "PR"]
    session_names = set()

    for folder in subfolders:
        subfolder_path = os.path.join(excel_output_files, folder)
        if not os.path.exists(subfolder_path):
            continue

        for f_name in os.listdir(subfolder_path):
            # Must start with MTF134 or BSB273, contain the cohort code, match pattern
    ...
    # session columns begin at col=4
    ...
    # Gather all sessions in the sheet
    max_col_now = behavior_ws.max_column
    all_sessions_in_sheet = [
        behavior_ws.cell(row=2, column=c).value for c in range(FIRST_SESSION_COL, max_col_now + 1)
    ]

    # Insert 'Start Date' in row=1 for each session
    for s_name in all_sessions_in_sheet:
        if not s_name:
            continue
        for folder in subfolders:
            sfpath = os.path.join(excel_output_files, folder)
            if not os.path.exists(sfpath):
                continue

            for f_name in os.listdir(sfpath):
                if (f_name.startswith("MTF134") or f_name.startswith("BSB273")) \
                   and s_name in f_name \
                   and f_name.endswith("_output.xlsx") \
                   and cohort_pattern.search(f_name):

                    session_file_path = os.path.join(sfpath, f_name)
                   ...
                   and cohort_pattern.search(f_name):
                    ...
                    if reward_row and subject_row:
                        ...
                        for rat_idx, rat_id in enumerate(subject_data[1:], start=1):
                            just_rats = [tp[0] for tp in original_rat_rfid_dg]
                            if rat_id in just_rats:
    ...
    if behavior_ws.cell(active_header_row, 1).value != "Rat":
        ...
        behavior_ws.cell(active_header_row, 4, "Experiment Group")
        roff = 1
        for (rat_val, rfid_val, drug_val, experiment_val) in original_rat_rfid_dg:
            ...
            behavior_ws.cell(active_header_row + roff, 4, experiment_val)
            roff += 1
            ...
            for f_name in os.listdir(sfpath):
                if (f_name.startswith("MTF134") or f_name.startswith("BSB273")) \
                   ...
                   and cohort_pattern.search(f_name):
                    ...
                    for row_cells in session_ws.iter_rows(min_col=1, max_col=1):
                        if row_cells[0].value == "Active Lever Presses":
    ...
    if behavior_ws.cell(inactive_header_row, 1).value != "Rat":
        ...
        behavior_ws.cell(inactive_header_row, 3, "Experiment Group")
        roff = 1
        for (rat_val, rfid_val, drug_val, experiment_val) in original_rat_rfid_dg:
            ...
            behavior_ws.cell(inactive_header_row + roff, 4, experiment_val)
            roff += 1
            ...
            for f_name in os.listdir(sfpath):
                if (f_name.startswith("MTF134") or f_name.startswith("BSB273")) \
                   ...
                   and cohort_pattern.search(f_name):
                    ...
                    subject_row_in_file = None
                    for row_cells in session_ws.iter_rows(min_col=1, max_col=1):
                        if row_cells[0].value == "Inactive Lever Presses":
    ...
    if behavior_ws.cell(breakpoint_header_row, 1).value != "Rat":
        ...
        for (rat_val, rfid_val, drug_val, experiment_val) in original_rat_rfid_dg:
            ...
            behavior_ws.cell(breakpoint_header_row + roff, 4, experiment_val)
            roff += 1
    ...
    # -------------------------------------------------------------------------
    # 12) FILL EXIT DATA for each rat in exit_data_map
    # -------------------------------------------------------------------------
    max_row_behavior = behavior_ws.max_row
    for exit_rat, (day_v, code_v, notes_v, last_good_v) in exit_data_map.items():
        found_any = False
        for row_b in range(7, max_row_behavior + 1):
            b_rat = behavior_ws.cell(row=row_b, column=1).value
            if b_rat and str(b_rat).strip() == exit_rat:
                ...
                behavior_ws.cell(row=row_b, column=last_good_col, value=last_good_v)
                found_any = True
        if not found_any:
            print(f"Rat {exit_rat} not found in behavior sheet.")

    # -------------------------------------------------------------------------
    # 13) DAILY ISSUES & NOTES => Rows 3 & 4
    # -------------------------------------------------------------------------
    behavior_ws["A3"] = "Daily Issues"
    behavior_ws["A4"] = "Notes"

    # Clear row=3 & 4 for columns=4..end
    max_col_now = behavior_ws.max_column
    ...
    if os.path.exists(daily_issues_file_path):
        ...
        # Build session->column map in uppercase
        session_col_map = {}
        ...
        for trial_id_upper, issues_list in daily_issues_map.items():
            if trial_id_upper in session_col_map:
                ...
                combined_issues = "\n".join(issues_list)
                behavior_ws.cell(row=3, column=col_idx, value=combined_issues)

        # Put all notes in row=4
        for trial_id_upper, notes_list in daily_notes_map.items():
            if trial_id_upper in session_col_map:
                col_idx = session_col_map[trial_id_upper]
                combined_notes = "\n".join(notes_list)
                behavior_ws.cell(row=4, column=col_idx, value=combined_notes)
    else:
    ...
    print(f"Data merged (for {cohort_str} Oxy) with Experiment Group column. "
          f"Saved at: {behavior_sheet_path} (unsorted).")

    # -------------------------------------------------------------------------
    # 15) SORT COLUMNS BY DATE & REMOVE OLD SHEET
    # -------------------------------------------------------------------------
    sort_columns_by_date(behavior_wb, source_sheet_name="Behavior Data", sorted_sheet_name="Sorted Data")
    behavior_wb.save(behavior_sheet_path)
