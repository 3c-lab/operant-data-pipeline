...
def parse_contents(contents, filename, date):
    ...
    try:
        ...
        # Processing for info section
        fname = df.Filename.iloc[0].split('\\')[-1]
        initiated = df['Start Datetime'].min()
        terminated = df['End Datetime'].max()
        date = datetime.datetime.fromtimestamp(date)
        info = OrderedDict([
            ("Info", ["File Title", "Filename", "Duration", "Last Modified"]),
            (" ", [filename, fname, "{} --- {}".format(initiated,terminated), date])
            ])
        ...
        subjects, n_active, n_inactive, n_reward, n_timeout = [],[], [], [], []
        ...
        dbc.Row([
            dbc.Col([
                dbc.Card([
                    dbc.CardBody([
                        html.H5('Active'),
                        html.H2(df['Active Lever Presses'].sum())
                    ], style={'textAlign':'center'})
                ], color='primary', inverse=True),
            ], width=3),
            ...
            dbc.Col([
                dbc.Card([
                    dbc.CardBody([
                        html.H5('Inactive'),
                        html.H2(df['Inactive Lever Presses'].sum())
                    ], style={'textAlign':'center'})
                ], color='primary', inverse=True),
            ], width=3),
